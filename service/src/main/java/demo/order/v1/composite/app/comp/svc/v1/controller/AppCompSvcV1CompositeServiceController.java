package demo.order.v1.composite.app.comp.svc.v1.controller;
...
public class AppCompSvcV1CompositeServiceController {
  ...
  public ResponseEntity<List<List<Order>>> groupLogicalShipments(
    ...
    List<List<Order>> data = appCompSvcV1CompositeService.groupLogicalShipments(lineItemArray);
    if (data != null) {
  ...
  /** */
  @PostMapping(
      value = "/op/v1/order/release/releasegroups",
      consumes = {MediaType.APPLICATION_JSON_VALUE},
      produces = {MediaType.APPLICATION_JSON_VALUE})
  @ApiOperation(value = "", tags = "", response = Order.class)
  @ApiResponses({@ApiResponse(code = 200, message = "OK")})
  public ResponseEntity<List<List<Order>>> splitLineItems(
      @ApiParam(value = "lineItemArray", required = true)
          @Validated({CreateGroup.class})
          @RequestBody
          List<Order> lineItemArray) {

    List<List<Order>> data = appCompSvcV1CompositeService.splitLineItems(lineItemArray);
    if (data != null) {
      return new ResponseEntity<>(data, HttpStatus.OK);
    } else {
      return new ResponseEntity<>(HttpStatus.NOT_FOUND);
